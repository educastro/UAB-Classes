...
public class BFSAdjacencyMatrix {
	...
	public BFSAdjacencyMatrix(UndirectedGraphAdjacencyMatrix G, int s) {
	...
	private void bfs(UndirectedGraphAdjacencyMatrix G, int s) {
		Queue<Integer> q = new ArrayDeque<Integer>();
		for (int v = 0; v < G.V; v++) distTo[v][v] = INFINITY;
		distTo[s][s] = 0;
		marked[s][s] = true;
		q.add(s);
	...
	public void printPath(UndirectedGraphAdjacencyMatrix undirectedGraph, int s, int v){
		if (v == s)
			System.out.print(s);
		else if (edgeTo[s][v] != 0)
			System.out.print("no path");
		else{
			printPath(undirectedGraph, s, edgeTo[s][v]);
		}
		System.out.print(v);	
	...
	public void printPath(UndirectedGraphAdjacencyMatrix undirectedGraph){
