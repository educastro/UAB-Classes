...
public class ExtractMin {
	     ...
	     */
	    public ExtractMin(int NMAX) {
	        if (NMAX < 0) throw new IllegalArgumentException();
	        this.NMAX = NMAX;
	        keys = new double[NMAX + 1];    // make this of length NMAX??
	        pq   = new int[NMAX + 1];
	    ...
	    public int delMin() { 
	        ...
	        sink(1);
	        qp[min] = -1;            // delete
	        keys[pq[N+1]] = 0;    // to help with garbage collection
	        pq[N+1] = -1;            // not needed
	        return min; 
	    }
	     ...
	     */
	    @Deprecated public void change(int i, double key) {
	        changeKey(i, key);
	    }
	    ...
	    public void decreaseKey(int i, double key) {
	        if (i < 0 || i >= NMAX) throw new IndexOutOfBoundsException();
	        if (!contains(i)) throw new NoSuchElementException("index is not in the priority queue");
	        if (keys[i] <= key)  throw new IllegalArgumentException("Calling decreaseKey() with given argument would not strictly decrease the key");
	        keys[i] = key;
	        swim(qp[i]);
	    }
	    ...
	    public void increaseKey(int i, double key) {
	        if (i < 0 || i >= NMAX) throw new IndexOutOfBoundsException();
	        if (!contains(i)) throw new NoSuchElementException("index is not in the priority queue");
	        if (keys[i] >= key) throw new IllegalArgumentException("Calling increaseKey() with given argument would not strictly increase the key");
	        keys[i] = key;
	        sink(qp[i]);
	    }
